...
//    patternBit = 1;
    ...
    while (1)
    {
    	if (tickCounter != prevCounter)
    	{
    		...
    		{
    			currentState = pgm_read_byte(&(STATES[currentState].button));
    			tickCounter = 0; tickCounter = 0;
//    			patternTickCounter = 0; patternBit = 0;
    		}
    		else if (prevCounter >= pgm_read_dword(&(STATES[currentState].time)))
    		{
    			currentState = pgm_read_byte(&(STATES[currentState].time_over));
    			tickCounter = 0; tickCounter = 0;
//    			patternTickCounter = 0; patternBit = 0;
    		}
    		...
    		if (PINB & PWR_IN)
    		{
				if (pgm_read_byte(&(STATES[currentState].red)) & patternBit) {port |= LED_OUT_R;}
				if (pgm_read_byte(&(STATES[currentState].green)) & patternBit) {port |= LED_OUT_G;}
    		}
    		if (fanTickCounter > 0) {
    			if (pgm_read_byte(&(STATES[currentState].fan))) {port |= FAN_OUT;}
    			fanTickCounter --;
    		}
    		PORTB = port;
